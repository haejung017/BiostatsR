#Competing Risk Model log likelihood function
loglik_Comp_Timedep=function(theta, data, data2, agemin, frailty=FALSE, weight=FALSE, reccur.type, time.dep.cov.type){
  ...
  if(time.dep.cov.type=="ED"){
    #phiv <- exp(theta[c(16,17,18)])
    phiv <- exp(theta[c(14,15,16)])
    phiv= c(phiv,0)
    phi_sc = phiv[c(1:3)]; phi_sc = c(phi_sc,rep(0,3))
    #phi_or = phiv[4]; phi_or = c(phi_or, 0)
    phi_or = 0; phi_or = c(phi_or, 0)
  }else if(time.dep.cov.type=="CO"){
    #phiv = c( exp(theta[c(20,21,22,23)]), theta[c(16,17,18,19)]   )
    phiv = c( c(exp(theta[c(19,20,21)]),0) , c(theta[c(16,17,18)],0)   ) # BO PE version
    
    phi_sc = phiv[c(1:3,5:7)] # phi 1~3 rate parameter, phi 4~6 intercept parameter
    phi_or = c(0,0)  #BO PE version
    #phi_or = phiv[c(4,8)]     # phi_or 1 rate parm, phi_or 2 intercept  
  ...
  sc_vec_bc[sc_vec==2] <- ifelse(reccur.type=="CM", beta.sc1_1 , 0) + Z_SC2[sc_vec==2]*beta.sc2_1 + phi_sc[5]
  sc_vec_bc[sc_vec==3] <- ifelse(reccur.type=="CM", beta.sc1_1 + beta.sc2_1 , 0) + Z_SC3[sc_vec==3]*beta.sc3_1 + phi_sc[6]
  ...
  sg_vec_bc[sc_vec==2] <- ifelse(reccur.type=="CM", beta.sc1g_1  , 0) + beta.sc2g_1
  sg_vec_bc[sc_vec==3] <- ifelse(reccur.type=="CM", beta.sc1g_1 + beta.sc2g_1 , 0) + beta.sc3g_1
  ...
  # sum of log-hazard
  sum1 = sum((      wt*ex1*(logh1.1 + (delta_OR*Z_OR*beta.or1 + phi_or[2]) + sc_vec_bc + sg_vec_bc + os_vec_bc ) +
                wt*(1-ex1)*(logh1.0 + (delta_OR*Z_OR*beta.or1 + phi_or[2]) + sc_vec_bc + os_vec_bc))[status==1], na.rm=TRUE) 
  ...
  if(reccur.type=="CM"){
    ...
    #sum4 = apply(mat_all, 1, func_all_IS, cest=cest, bref=bref, beta.gen1=beta.gen1, beta.gen2=beta.gen2, beta.gen3=beta.gen3) 
       ...
       CH_ov=CumH_c_ED(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=FALSE, type=2,phi=c(0,0,0,0))
        CH_d=CumH_c_ED(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=FALSE, type=3,phi=c(0,0,0,0))
   ...
   # sum4 = apply(mat_all, 1, func_all_IS_ED, cest=cest, bref=bref, beta.gen1=beta.gen1, beta.gen2=beta.gen2, beta.gen3=beta.gen3, phiv=phiv)     
      ...
      sum4=k
       ...
       testm <- mat_all
       testm[is.na(testm)] <-   99
       CH_bc=CumH_c_CO(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=FALSE, type=1,phi=phiv)
       CH_ov=CumH_c_CO(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=FALSE, type=2,phi=phiv)
        CH_d=CumH_c_CO(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=FALSE, type=3,phi=phiv)
       CH0_bc <- (1-testm[,12])*CH_bc[,1]
       CH1_bc <-    testm[,12]*exp(beta.gen1)*CH_bc[,2]
       CH0_ov <- (1-testm[,12])*CH_ov[,1]
       CH1_ov <-    testm[,12]*exp(beta.gen2)*CH_ov[,2]
  ...
  if(frailty==TRUE){ # Gamma-frailty
    ...
    sum4.1 <- sum(wtp*(lfactorial(fp[1]+df1-1)-(df1-1)*log(fp[1])-lfactorial(fp[1]) + (-fp[1]-df1)*log(1+(Hfam1)/fp[1])), na.rm=T)
    sum4.2 <- sum(wtp*(lfactorial(fp[2]+df2-1)-(df2-1)*log(fp[2])-lfactorial(fp[2]) + (-fp[2]-df2)*log(1+(Hfam2)/fp[2])), na.rm=T)
    #sum4.3 <- sum(wtp*(lfactorial(fp[3]+df3-1)-(df3-1)*log(fp[3])-lfactorial(fp[3]) + (-fp[3]-df3)*log(1+(Hfam3)/fp[3])), na.rm=T)
  ...
  if(frailty==TRUE){
    logS <- log((1-CH1_bc/fp[1])^(-fp[1])) + log((1-CH1_ov/fp[2])^(-fp[2])) +  CH1_d 
  }else {
    logS <- logasc0
  }
  ...
  logasc1 <- apply(mat_p1, 1, proband_penf, type=1, affp=TRUE, cest=cest, bref=bref)
    ...
    if(time.dep.cov.type=="PE"){
  logasc1 <- apply(mat_p1, 1, proband_penf_ED, type=1, affp=TRUE, cest=cest, bref=bref, phi=c(0,0,0,0),frailty=frailty, fp=fp)
    } else if (time.dep.cov.type=="ED"){
  logasc1 <- apply(mat_p1, 1, proband_penf_ED, type=1, affp=TRUE, cest=cest, bref=bref, phi=phiv, frailty=frailty, fp=fp)
    } else if (time.dep.cov.type=="CO"){
  logasc1 <- apply(mat_p1, 1, proband_penf_CO, type=1, affp=TRUE, cest=cest, bref=bref, phi=phiv, frailty=frailty, fp=fp)
    }
  }
  ...
  if(time.dep.cov.type=="PE"){
    logasc2 <- apply(mat_p2, 1, proband_penf_ED, type=2, affp=TRUE, cest=cest, bref=bref, phi=c(0,0,0,0),frailty=frailty, fp=fp)
  } else if (time.dep.cov.type=="ED") {
    logasc2 <- apply(mat_p2, 1, proband_penf_ED, type=2, affp=TRUE, cest=cest, bref=bref, phi=phiv, frailty=frailty, fp=fp)
  } else if (time.dep.cov.type=="CO") {
    logasc2 <- apply(mat_p2, 1, proband_penf_CO, type=2, affp=TRUE, cest=cest, bref=bref, phi=phiv, frailty=frailty, fp=fp)
  }
  }
...
integrand_IS <- function(u,v1,v2,est, bref,affp,type,beta_vec1,beta_vec2,beta_vec3,intg,intw,frailty=FALSE,fp=c(1,1,1)){
  u <- u
  #  print(u)
  # v1 = v1[!is.na(v1)]
  # v2 = v2[!is.na(v2)]
...
integrand_IS_ED <- function(u,v1,v2,est, bref,affp,type,beta_vec1,beta_vec2,beta_vec3,intg,intw,phi,phisc,lb,frailty=frailty,fp=c(20,20,20)){
  u <- u
  #  print(u)
  # v1 = v1[!is.na(v1)]
  # v2 = v2[!is.na(v2)]
...
# Interval specific hazard + CO
proband_penf_CO <- function(v, cest, bref, affp, type, phi, frailty, fp){
  v1 <- v[1:5]
  v2 <- v[6:10]
  terminal_time = v[11]
  v1 = v1[v1<terminal_time]
  ...
  beta_sc=0; beta_or=0; lowsc=0; lowor=0; phisc=0; beta_sc_sept=0; beta_or_sept=0;
  for(i in 1:length(v1)){
    ...
    if(i!=1){
      ...
      if(v2[i-1]==1 | v2[i-1]==2 | v2[i-1]==3 ){
        ...
        beta_sc = beta;
        lowsc = v2[i-1];
        beta_sc_sept = coeff_c_CO_int(v2[i-1],type=1, affp, phi);
      }else if(v2[i-1]==4){
        beta_or = beta;
        lowor = v2[i-1];
        beta_or_sept = coeff_c_CO_int(v2[i-1],type=1, affp, phi);
      }
    }else{
      beta = 0;
      beta_sc = beta;
    ...
    beta1 <- c(beta_sc,lowsc,beta_or,lowor); beta1_sept=c(beta_sc_sept,beta_or_sept)
    beta2 <- ifelse(i==1, 0, coeff_c_ED(v2[i-1],type=2, cest=cest, affp=affp) )
    beta3 <- ifelse(i==1, 0, coeff_c_ED(v2[i-1],type=3, cest=cest, affp=affp) )
    ...
    # }
    low=ifelse(i==1,0,v1[i-1])
    Penpiece = penf.comp_CO(f=integrand_IS_CO, v1=v1, v2=v2, est=cest,bref=bref, affp=affp,beta_vec1=beta1,beta_vec1_sept=beta1_sept,beta_vec2=beta2,beta_vec3=beta3,
                            intg=intg,intw=intw,type=type,phi=phi,phisc=phisc,lb=low,frailty=frailty,fp=fp,lower=low,upper=v1[i])
    j = c(j,Penpiece)
  }
  j = sum(j,na.rm = TRUE)
...
penf.comp_CO <- function(f,v1,v2,est,bref, affp,type,beta_vec1,beta_vec1_sept,beta_vec2,beta_vec3,intg,intw,phi,phisc,lb,frailty,fp,lower,upper){  
  integrate(f,v1,v2,est=est,bref=bref, affp=affp,beta_vec1=beta_vec1,beta_vec1_sept=beta_vec1_sept,
            beta_vec2=beta_vec2,beta_vec3=beta_vec3,type=type,intg=intg,intw=intw,
            phi=phi,phisc=phisc,frailty=frailty,fp=fp,lb=lb,lower=lower,upper=upper)$value
}
integrand_IS_CO <- function(u,v1,v2,est, bref,affp,type,beta_vec1,beta_vec1_sept,beta_vec2,beta_vec3,intg,intw,phi,phisc,lb,frailty=frailty,fp=c(20,20,20)){
  u <- u
  #  print(u)
  # v1 = v1[!is.na(v1)]
  # v2 = v2[!is.na(v2)]
  ...
  if(type==1){
    xbeta = est[7] + (est[8] + bref[1])*zsc1*zsc1td +(est[9] + bref[2])*zsc2*zsc2td+(bref[3] + est[10])*zsc3*zsc3td + 
      est[11]*zor*zortd + bref[4]*zor*zsc1 + bref[5]*zor*zsc2  + bref[6]*zor*zsc3 + beta_vec1_sept[1]+beta_vec1_sept[2]
    haz=(est[1]^est[2])*est[2]*(u)^(est[2]-1)*exp(xbeta)
    #print(c(xbeta, est[7]+sum(beta_vec1)))
  }else if(type==2){
    haz=(est[3]^est[4])*est[4]*(u)^(est[4]-1)*exp(est[12]+est[13]*zsc1ov)  
  ...
  H1 = - (1-zbr)*exp(est[7])*sapply(u, chaz, lam1=est[1],rho1=est[2],
                                    betasc=beta_vec1[1],betaor=beta_vec1[3], phisc=phisc,phior=phi[4],
                                    lowsc=beta_vec1[2], bsc=beta_vec1_sept[1], bor=beta_vec1_sept[2], lowor=beta_vec1[4],low=lb)
  H2 = - (1-zor)*(est[3]*u)^est[4]*exp(est[12]+ sum(beta_vec2))
  H3 = - (est[5]*u)^est[6]*exp(est[14]+ sum(beta_vec3) ) 
...
#Non-Competing Risk Model loglikelihood function (Breast Cancer Model)
loglik_Comp_Timedep_NC=function(theta, data, data2, agemin, frailty=FALSE, weight=FALSE, reccur.type, time.dep.cov.type){
  ...
  # time varying covariate type
  if(time.dep.cov.type=="ED"){
    #phiv <- exp(theta[c(8,9,10,11)])
    phiv = c(exp(theta[c(8,9,10)]),0)
    phi_sc = phiv[c(1:3)]; phi_sc = c(phi_sc,rep(0,3))
    phi_or = phiv[4]; phi_or = c(phi_or, 0)

  }else if(time.dep.cov.type=="CO"){
    phiv = c( theta[c(8,9,10,11)], exp(theta[c(12,13,14,15)])  )
    phi_sc = phiv[c(5:7,1:3)]
    phi_or = phiv[c(8, 4)]
    #phi_or = 0
  }else{ #PE
    phi_sc=rep(0,6)
  ...
  if(time.dep.cov.type=="ED" | time.dep.cov.type=="CO"){
  ...
  # sum of survival until event time for all the individuals in the data
  lambda2=0;rho2=0;lambda3=0;rho3=0;
  cest <- c(lambda1,rho1,
            lambda2,rho2,
            lambda3,rho3,
            beta.gen1,#7
  ...
  if(frailty==TRUE){ # Gamma-frailty
    ...
    sum4 <- sum(wt*sum4,na.rm = TRUE)
    loglik = sum1+sum4 # numerator in loglikelihood
  }
  ...
  if(reccur.type=="CM"){
    ...
    if(time.dep.cov.type=="PE"){
      testm <- mat_p1
      testm[is.na(testm)] <- 99
      CH_bc=CumH_c_ED(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=TRUE, type=1, phi = c(0,0,0,0))
      CH1_bc <- exp(beta.gen1)*CH_bc[,2]
      logasc1 <- CH1_bc
    } else{
      testm <- mat_p1
      testm[is.na(testm)] <- 99
      CH_bc=CumH_c_ED(v1= testm[,1:5], v2=testm[,6:10], u = testm[,11],cest=cest, bref=bref, affp=TRUE, type=1, phi = phiv)
      CH1_bc <- exp(beta.gen1)*CH_bc[,2]
      logasc1 <- CH1_bc
    }
  }
  ...
  logasc1 <- sum(wtp1*(log(1-S)),na.rm=TRUE)
