...
simfam.cmp <- function(N.fam=779, iest, frailty.dist, mrate){
  ...
  simdat=familyDesign(n=N.fam, affectnum=1,  m.carrier= 1, depend=c(iest[9],iest[10]), 
                      vbeta=iest[5:8], parms=iest[1:4], variation="none", 
...
familyStructure <-
            ...
            age1=c(65,2.5), age2=c(45,2.5), agemin=14)
  {
    ...
    if (SecNum > 0) { ## at least one sample in second generation
      ...
      a=(sd/m)^(-1.086)
      b=m/gamma(1+1/a)
      tvc<-rweibull(length(inID), shape=a, scale=b )
      #------------------------------------------------------------------------------
      ...
      affected <- (proband==1)
        ...
        xv2beta <- gender*vbeta3 + majorgene*vbeta4
...
fgene <-function(base.dist, affage, affsex, variation="secondgene", parms, vbeta, alpha1,alpha2,
           pg=0, m.carrier=0, dominant.m=TRUE, aq,tvctype,tvc){
...
integrand.penf.cmp=function(u,delta, base.dist, parms, xbeta1, xbeta2)
{
  ...
  cH_PE(a=0,b=u,parm=c(parms[1:2],bz1))
