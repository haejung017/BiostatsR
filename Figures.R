...
setwd("E:/School/MSC/latex_thesis_jay/revise/applicationsection/Modelling/Plots")
est1 <-readRDS("BRCA1est") # parms estimates
est1var <-readRDS("BRCA1est_robustvar") # parms robust variance 

estimate=est1$par

baseparms=exp(estimate[1:6])
regress=estimate[7:13]
other=exp(estimate[14:length(estimate)])

hazard_base=function(parms,cause,from,to){
if(cause==1) index=c(1,2)
if(cause==2) index=c(3,4)
if(cause==3) index=c(5,6)
t=seq(from=from-16, to=to-16, by=0.01)
haz <- (parms[index[1]]^parms[index[2]])*parms[index[2]]*t^(parms[index[2]]-1)
return(list(t=t,haz=haz))
}
regress_factor=function(regress, other, from,to,tvc){
  t=seq(from=from-16, to=to-16, by=0.01)
  exp(regress[2]*exp(-other[1]*(t-(tvc-16))))
}

plot(h1_base$t,h1_base$haz, type="l", xlim=c(0,55), ylim=c(0,0.1))
lines(h1_base_dotted$t,h1_base_dotted$haz,lty=2)
lines(h2_base$t,h2_base$haz)

lines(h1_base_dotted$t,
      h1_base_dotted$haz*regress_factor(regress,other,from=40,to=70,tvc=40))


# Baseline hazards + regression coefficients
h1_base=hazard_base(baseparms,1,16,40)
h1_base_dotted=hazard_base(baseparms,1,40,70)
h2_base=hazard_base(baseparms,2,16,70)
...
PrintPen=function(modelobject, RobustVar, eventage, eventind, G, type=1, res=100, data){
  ...
  Pen1_B=Computation(type=1,G=0)
  Pen1_O=Computation(type=2,G=0)
  Pen1_D=Computation(type=3,G=0)
  t=t+16
  plotdat1=cbind(t,Pen1_D,"Death")
  plotdat2=cbind(t,Pen1_B,"Breast Cancer")
  plotdat3=cbind(t,Pen1_O,"Ovarian Cancer")
  plotdat=rbind(plotdat1,plotdat2,plotdat3)
  plotdat=as.data.frame(plotdat)
  colnames(plotdat)=c("t","e","Event")
  new=NULL
  for(i in t){
    k=plotdat[plotdat$t==i,]
    new=rbind(new,k)
  }
  new$t<-as.numeric(as.character(new$t))
  new$e<-as.numeric(as.character(new$e))
  # some variables for ggplot
  y.limits = c(0,1)
  x.limits = c(24,70)
  g.title= "Stacked Plot of CIFs"
  new$Event=factor(new$Event , levels=levels(new$Event)[c(3,1,2)])
  ##output
  plot1=    ggplot(new, aes(x=t,y=e,fill=Event))+ 
    geom_area(position = 'stack', size=.2, alpha=.7)+
  # Themes
  scale_x_continuous(name = "Ages in years", limits=x.limits) +
  scale_y_continuous(name = "Penetrance", limits=y.limits) + # ylab
#ggtitle(g.title) + #title
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5,size=15), # Centering of the title
          axis.text.x = element_text(face="bold",size=18),  # Change axis ticks labels
          axis.text.y = element_text(size=18),
          axis.title.x = element_text(size=18),
          axis.title.y = element_text(size=18))
plot1=plot1 + theme(legend.title = element_text(size=20, color = "black"),
  ...
  Pen1_B=Computation(type=1,G=1)
  Pen1_O=Computation(type=2,G=1)
  Pen1_D=Computation(type=3,G=1)
  plotdat1=cbind(t,Pen1_D,"Death")
  plotdat2=cbind(t,Pen1_B,"Breast Cancer")
  plotdat3=cbind(t,Pen1_O,"Ovarian Cancer")
  plotdat=rbind(plotdat1,plotdat2,plotdat3)
  ...
  new$Event=factor(new$Event , levels=levels(new$Event)[c(3,1,2)])
  ##output
plot2=  ggplot(new, aes(x=t,y=e,fill=Event))+ 
    geom_area(position = 'stack', size=.2, alpha=.7)+
    # Themes
    ...
    theme(plot.title = element_text(hjust = 0.5,size=15), # Centering of the title
          ...
          axis.title.x = element_text(size=18),
          axis.title.y = element_text(size=18))
plot2=plot2 + theme(legend.title = element_text(size=20, color = "black"),
                    legend.text=element_text(size=20),
        legend.justification=c(0,1), 
        legend.position=c(0.05, 0.95),
        legend.background = element_blank(),
...
pdf("StackedPlot.pdf",width = 14, height = 7,pointsize=10)
...
integrand_IS_ED <- function(u,v1,v2,est,affp,type,beta_vec1,beta_vec2,beta_vec3,phi,phisc,lb,H,frailty=frailty,fp=c(20,20,20)){
  ...
  if(type==1){
    ...
    haz=(est[5]^est[6])*est[6]*(u)^(est[6]-1)*exp(est[13])  
  }
  ...
  if(frailty==TRUE){
    if(type==1){
      ...
      f = haz*((1-H1/fp[1])^(-fp[1]))*((1-H2/fp[2])^(-fp[2]))*(exp(H3))
    }
  } else{
    Hp <- H1+H2+H3
    f = haz*exp(Hp) # h * S   
