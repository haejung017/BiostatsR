...
simfam.cmp <- function(N.fam=779, iest, frailty.dist, tvctype="PE",mrate){
  
  #set.seed(20)
  if(tvctype=="TI")vbeta=iest[5:7]
  if(tvctype=="PE")vbeta=iest[5:7]
  if(tvctype=="ED")vbeta=c(iest[5:7],iest[10])
  if(tvctype=="CO")vbeta=c(iest[5:7],iest[10:11]) #1 TD
  
  simdat=familyDesign(n=N.fam, affectnum=1,  m.carrier= 1, depend=c(iest[8],iest[9]), 
                      vbeta=vbeta, parms=iest[1:4], variation="none", 
                      base.dist="Weibull", frailty.dist=frailty.dist,
                      allelefreq=0.0021, mrate=mrate, agemin=20,tvctype=tvctype)
  ...
  #-----------------some change in data-------------------------#
# 
  # delind=  sample(simdat$indID,round(N.fam*5))
  # simdat=simdat[!(simdat$status==0&simdat$indID%in%delind&simdat$proband==0),]

  # delfam2=sample(1:N.fam,round(N.fam*0.1))
  # simdat=simdat[!(simdat$famID%in%delfam2&simdat$proband!=1),]
  # 
  # delfam1=sample(1:N.fam,round(N.fam*0.5))
  # simdat=simdat[!(simdat$famID%in%delfam1&simdat$generation==3),]
  # 
  #   # 
  # 
  #-------------------------------------------------------------#
            ...
            age1=c(65,2.5), age2=c(45,2.5), agemin=14,tvctype="PE")
  {
    ...
    if (SecNum > 0) { ## at least one sample in second generation
      ...
      ## Gamma Frailty (with mean 1, variance=depend)
        ...
        alpha1 <- rgamma(1, shape=depend[1], scale=1/depend[1])
        alpha2 <- rgamma(1, shape=depend[2], scale=1/depend[2])}
      else alpha1 =alpha2= 1
      ...
      tvc=runif(length(pos),min=5,max=70)
      # tvc<-rnorm(length(pos), mean=30, sd=7)
       tvc[tvc<=5]=150
      ...
      affected <- (proband==1)
        ...
        ageonset[genepos] <- apply(cbind(xv1beta[genepos],xv2beta[genepos],affage.min,tvc[genepos],uni), 1, inv.survp, base.dist=base.dist, parms=parms, alpha1=alpha1, alpha2=alpha2, bz=bz,tt=tvctype,eta=eta)+agemin
        
        if(affage.min< tvc[genepos]){
          pen1 <- penf.cmp(affage.min, delta=1, base.dist=base.dist, parms=c(parms,alpha1,alpha2), xbeta1=xv1beta[genepos],xbeta2=xv2beta[genepos])
          pen2 <- penf.cmp(affage.min, delta=2, base.dist=base.dist, parms=c(parms,alpha1,alpha2), xbeta1=xv1beta[genepos],xbeta2=xv2beta[genepos])
        }else{
          pen1<- penf.cmp(tvc[genepos], delta=1, base.dist=base.dist, parms=c(parms,alpha1,alpha2), xbeta1=xv1beta[genepos],xbeta2=xv2beta[genepos])+
                  penf.cmp2(affage.min,tvc= tvc[genepos], delta=1, base.dist=base.dist, parms=c(parms,alpha1,alpha2), xbeta1=xv1beta[genepos],xbeta2=xv2beta[genepos],bz=bz,tt=tvctype,eta=eta)
          pen2<- penf.cmp(tvc[genepos], delta=2, base.dist=base.dist, parms=c(parms,alpha1,alpha2), xbeta1=xv1beta[genepos],xbeta2=xv2beta[genepos])+
                penf.cmp2(affage.min,tvc= tvc[genepos], delta=2, base.dist=base.dist, parms=c(parms,alpha1,alpha2), xbeta1=xv1beta[genepos],xbeta2=xv2beta[genepos],bz=bz,tt=tvctype,eta=eta)
        }
        status[genepos] <- ifelse(runif(length(genepos)) < pen1/(pen1+pen2), 1, 2)

        
        # if(tvc[genepos]<affage.min){H1 = (cumhaz(base.dist, tvc[genepos] ,parms=parms[1:2])+cH_ED(a=tvc[genepos],b=affage.min,parm=c(parms[1:2],bz[1],eta[1],eta[2])))*exp(xv1beta[genepos])*alpha1 #cause 1
        # }else{H1 = (cumhaz(base.dist, affage.min ,parms=parms[1:2]))*exp(xv1beta[genepos])*alpha1}
        # H2 = exp(xv2beta[genepos])*cumhaz(base.dist, affage.min ,parms=parms[3:4])*alpha2 #cause 2
        # status[genepos] <- ifelse(runif(length(genepos)) < H1/(H1+H2), 1, 2)
        ...
        # h1 <- hazards(dist=base.dist, t, parms[1:2])*exp(xv1beta[genepos]+ifelse(t<tsc,0,xzb1))*alpha1
        # h2 <- hazards(dist=base.dist, t, parms[3:4])*exp(xv2beta[genepos])*alpha2
        # status[genepos] <- ifelse(runif(length(genepos)) < h1/(h1+h2), 1, 2)
      ...
      tmpdata<-cbind(
        ...
        majorgene=disgene.m, secondgene=disgene.s, ageonset, currentage, time, status, mgene, relation, fsize, naff,tvc,TVCstatus)
...
#                            base.dist="Weibull", frailty.dist=frailty.dist,
#                            depend=depend, parms=parms, vbeta=vbeta, dominant.m=TRUE, dominant.s=TRUE,
#                            variation=variation, allelefreq=c(0.0021,0.2), mrate=mrate, age1=c(55,5),age2=c(45,5),
#                            agemin=16,tvctype=tvctype)
#       
#       if(is.null(attr(dat, "class"))){
#         # At least one parent in first gen and two sibs in the second gen should be affected
...
#     ## truncated negative binomial (>=2 and <=5) with prob=sibprob=0.4551, and rr=1
#     
#     #SecNum <-sample(c(1,2,3), 1, replace=TRUE, prob=c(0.4991, 0.2720+ 0.1482, 0.0807))
#     #SecNum <-sample(c(2,3,4,5), 1, replace=TRUE, prob=c(0.4991, 0.2720, 0.1482, 0.0807))
...
#         generation<-c(generation, c(2,0)) 
#         
#         #ThiNum<-sample(c(1,2,3), 1, replace=TRUE,prob=c(0.4991, 0.2720+ 0.1482, 0.0807))
#         #ThiNum<-sample(c(2,3,4,5), 1, replace=TRUE,prob=c(0.4991, 0.2720, 0.1482, 0.0807)) 
...
#       # tvc<-rweibull(nIndi, shape=a, scale=b )
#       tvc=runif(length(pos),min=5,max=70)
#       #tvc<-rnorm(length(pos), mean=24, sd=10)
#       #tvc[tvc<=5]=150
#       #tvc<-rtruncnorm(length(pos),a=0, b=70, mean=24, sd=10)
#       tvc[sample(1:length(pos),round(length(pos)/1.4))]=150
#       
#       
#       
#       ###  Generating the current age
...
#       genepos<-pos[generation==2] 
#       for (j in 1:length(genepos)) {
#         tmp<-rnorm(1, mean=age2[1], sd=age2[2])#rtruncnorm(1,a=agemin,b=min.page1-14, mean=age2[1]-j, sd=age2[2])
#         censorage[genepos[j]]<- tmp
#         #min.page2 <- min( c(tmp, censorage[generation==0]) )
#         sonpos<-pos[fatherID==indID[genepos[j]] | motherID==indID[genepos[j]]]
#         min.page2 <- min(censorage[indID==fatherID[sonpos[1]] | indID==motherID[sonpos[1]]])
...
#           censorage[sonpos[k]]<- rnorm(1, mean=age2[1], sd=age2[2])#rnorm(1,mean=min.page2-10-k, sd=1)#10
#         }#Close for for(k in 1:length(sonpos))
#       }#Close for for (j in 1:length(genepos))
#       #print(cbind(tvc+16,censorage))
#       
#       # genepos<-pos[generation==1]
#       # censorage[genepos]<- rnorm(length(genepos), mean=age1[1], sd=age1[2])
#       # min.page1<- min(censorage[genepos])
...
#       #   for (k in 1:length(sonpos)) {
#       #     #censorage[sonpos[k]]<- rtruncnorm(1,a=0, b=min.page2-14, mean=min.page2-20-k, sd=1)
#       #     censorage[sonpos[k]]<- rnorm(1,mean=min.page2-10-k, sd=1)#10
#       #   }#Close for for(k in 1:length(sonpos))
#       # }#Close for for (j in 1:length(genepos))
#       
#       
...
#       ageonset[genepos] <- apply(cbind(xv1beta[genepos],xv2beta[genepos],affage.min,tvc[genepos],uni), 1, inv.survp, base.dist=base.dist, parms=parms, alpha1=alpha1, alpha2=alpha2, bz=bz,tt=tvctype,eta=eta)+agemin
#       
#       # if(tvc[genepos]<affage.min){
#       #   H1 = (cumhaz(base.dist, tvc[genepos] ,parms=parms[1:2])+cH_CO(a=tvc[genepos],b=affage.min,parm=c(parms[1:2],bz[1],eta[1],eta[2])))*exp(xv1beta[genepos])*alpha1 #cause 1
#       # }else{
#       #   H1 = (cumhaz(base.dist, affage.min ,parms=parms[1:2]))*exp(xv1beta[genepos])*alpha1
#       # }
#       # H2 = exp(xv2beta[genepos])*cumhaz(base.dist, affage.min ,parms=parms[3:4])*alpha2 #cause 2
#       # 
#       # 
#       #pen1 <- penf.cmp(affage.min, delta=1, base.dist=base.dist, c(parms,alpha1,alpha2), xv1beta[genepos], xv2beta[genepos])
#       #pen2 <- penf.cmp(affage.min, delta=2, base.dist=base.dist, c(parms,alpha1,alpha2), xv1beta[genepos], xv2beta[genepos])
# 
#       status[genepos] <- ifelse(runif(length(genepos)) < pen1/(pen1+pen2), 1, 2)
#       #status[genepos] <- ifelse(runif(length(genepos)) < H1/(H1+H2), 1, 2)
#       
...
fgene <-function(base.dist, affage, affsex, variation="secondgene", parms, vbeta, alpha1,alpha2,
    ...
    eta=c(vbeta[4],vbeta[5])
      ...
      Ft <- 0
      for(i in c(0,1)){
        xbeta1 <- i*vbeta[2] #+alpha1
        xbeta2 <- i*vbeta[3] #+alpha2
        #print(c(affage,tvc))
        if(affage < tvc){
          # H1 = (cumhaz(base.dist, tvc,parms=parms[1:2])+cH_CO(a=tvc,b=affage,parm=c(parms[1:2],bz[1],eta[1],eta[2])))*exp(xbeta1)*alpha1 #cause 1
          # H2 = exp(xbeta2)*cumhaz(base.dist, affage,parms=parms[3:4])*alpha2 #cause 2
        Ft[i+1] <-                    penf.cmp(affage, delta=1, base.dist=base.dist, parms=c(parms,av), xbeta1=xbeta1,xbeta2=xbeta2)
                                      #penf.cmp(affage, delta=1, base.dist=base.dist, parms=c(parms,av), xbeta1=xbeta1,xbeta2=xbeta2)
                  #1-exp(-H1-H2)

                  #1-exp(-cumhaz(base.dist, affage, parms=parms[1:2])*exp(xbeta1)-cumhaz(base.dist, affage, parms=parms[3:4])*exp(xbeta2)) 
                  #+penf.cmp(affage, delta=2, base.dist=base.dist, parms=c(parms,av), xbeta1=xbeta1,xbeta2=xbeta2)
        }else{
          # H1 = exp(xbeta1)*cumhaz(base.dist, affage,parms=parms[1:2])*alpha1
          # H2 = exp(xbeta2)*cumhaz(base.dist, affage,parms=parms[3:4])*alpha2
        Ft[i+1]  <- penf.cmp(tvc, delta=1, base.dist=base.dist, parms=c(parms,av), xbeta1=xbeta1,xbeta2=xbeta2)+
                    penf.cmp2(affage, tvc, delta=1, base.dist=base.dist, parms=c(parms,av), xbeta1=xbeta1,xbeta2=xbeta2,bz=bz,tt=tt,eta=eta)
                    #penf.cmp(affage, delta=1, base.dist=base.dist, parms=c(parms,av), xbeta1=xbeta1,xbeta2=xbeta2)
                    # H1=-(cumhaz(base.dist, tvc, parms=parms[1:2])+cH_PE(tvc,affage,parm=c(parms[1:2],bz[1],eta[1],eta[2])))*exp(xbeta1)
                    # H2=-(cumhaz(base.dist, affage, parms=parms[3:4]))*exp(xbeta2)
                    #1-exp(H1+H2)
...
integrand.penf.cmp2=function(u,tvc,delta, base.dist, parms, xbeta1, xbeta2,bz,tt,eta)
{
  ...
  if(tt=="PE"){
  eta=eta*0;xzb1=bz[1]  
  H1 = (cumhaz(base.dist, u, parms=parms[1:2])-cumhaz(base.dist, tvc, parms=parms[1:2]))*exp(xbeta1+xzb1)*parms[5] #cause 1
  H2 = exp(xbeta2)*cumhaz(base.dist, u ,parms=parms[3:4])*parms[6] #cause 2
  }else if(tt=="ED"){
  xzb1=bz[1]*exp(-eta[1]*(u-tvc))#; xzb2=bz[2]*exp(-eta[2]*(u-tvc))
  H1 = sapply(u,cH_ED,a=tvc,parm=c(parms[1:2],bz[1],eta[1]))*exp(xbeta1)*parms[5] #cause 1
  H2 = exp(xbeta2)*cumhaz(base.dist, u ,parms=parms[3:4])*parms[6] #cause 2
  }else if(tt=="CO"){
  xzb1=bz[1]*exp(-eta[1]*(u-tvc))+eta[2] 
  H1 = (cumhaz(base.dist, tvc ,parms=parms[1:2])+sapply(u,cH_CO,a=tvc,parm=c(parms[1:2],bz[1],eta[1],eta[2])))*exp(xbeta1)*parms[5] #cause 1
  H2 = (cumhaz(base.dist, tvc ,parms=parms[3:4])+cumhaz(base.dist, u ,parms=parms[3:4]))*exp(xbeta2)*parms[6] #cause 2
  }  
...
surv.dist <- function(t, base.dist, parms, xbeta1,xbeta2, alpha1,alpha2, tvc,bz,tt,eta, res){
  ...
  Int=cH(A=tvc, B=t, parm=c(parms[1],parms[2],bz,eta[1],eta[2]), tt)
  
  # ind1<-which(t<tsc)
  # ind2<-which(t>=tsc)
  # chaz<-rep(NA,length(t))
  # 
  # if(length(ind1)>0)
  #   chaz[ind1]= (parms[1]*t[ind1])^parms[2]*exp(xbeta[ind1]) 
  # 
  # if(length(ind2)>0)
  #   chaz[ind2]= exp(xbeta[ind2])*((parms[1]*tsc[ind2])^parms[2] + Int[ind2]) 
  
  
    
  # if(tt=="PE"){
  # Int1=cH_PE(a=tvc, b=t[ind2], parm=c(parms[1:2],bz[1]))#;Int2=cH_PE(a=tvc, b=t[ind2], parm=c(parms[3:4],bz[2]))
  # }else if(tt=="ED"){
  ...
  if(length(ind1)>0){
    chaz1[ind1]= (parms[1]*t[ind1])^parms[2]*exp(xbeta1[ind1])*alpha1 
    chaz2[ind1]= (parms[3]*t[ind1])^parms[4]*exp(xbeta2[ind1])*alpha2
  }
  if(length(ind2)>0){  
    chaz1[ind2]= exp(xbeta1[ind2])*((parms[1]*tvc)^parms[2] + Int1)*alpha1
    chaz2[ind2]= exp(xbeta2[ind2])*(parms[3]*t[ind2])^parms[4]*alpha2
  }
  exp(-chaz1-chaz2)-res
}
